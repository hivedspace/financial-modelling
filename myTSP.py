...
def main():
    ...
    # Create random points
    print()
    no_points = int(input('Enter desired number of points: '))
    print()
    print('Registered points:', no_points)
    print()
    ...
    avg = input(
        'Do you want to calculate the average of several shortest routes (Y/N)? ')
    print()
    iterate = False
    if avg == 'Y':
        iterate = True
        ...
        print('Input not recognised, aborting.')
        return
    iterations = 1
    if iterate == True:
        print('Calculating average.')
        print('Enter desired iterations to average over:')
        iterations = int(input())
    elif iterate == False:
        print('Not calculating average.')
    ...
    for i in range(iterations):
        ...
        # Create depot (1st stop) as dummy entry by setting distances to 0
        for i in range(no_points):
            matrix[0][i] = 0
            matrix[i][0] = 0
    ...
    print('Average length of total path: ', avg_length / (iterations*100000), 'km.')
